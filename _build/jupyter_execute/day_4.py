...
Раз у нас задача классификации, и классов более двух, наша сеть нам должна отдавать вектор (неоткалиброванных вероятностей), где индекс наибольшего числа будет указывать на предсказанный класс. Делается это с помощью функции `softmax(x1, ..., xk) = (exp(x1) / sum(exp(xj)), ..., exp(xk) / sum(exp(xj))), j = 1..k`
...
А происходит вот что. При попадании серии в блок `GRU` делается проход по ней, в течении которого предыдущее состояние комбинируется с текущим элементом серии, и комбинация текущего и предыдущего состояния определяет выход. Веса определяются в процессе обучения так, чтобы дать минимум ошибки.

`GRU` таким образом также возвращает последовательности. Когда нужен только последний вектор, в `keras` можно задать слою настройку `return_sequences = False`. Давайте вернемся к нашей погоде в *Jena*.
